...
class GripperMarkers:
    ...
    def __init__(self, side_prefix):
        ...
        #stamped info
        self.marker_pose_stamp = PoseStamped()
        self.marker_pose_stamp.pose = self.marker_pose
        self.marker_pose_stamp.header.frame_id = '/base_link'
        ...
        #lab 11/12
        #rospy.logwarn(str(self.get_ee_pose())
        self.subscriber = rospy.Subscriber('ar_pose_marker', AlvarMarkers, self.recieved_marker_msg)

    def get_marker_pose_in_frame(self, target_frame = '/base_link'):
        return self.transform_to_stamp(self.marker_pose, '/base_link', target_frame)
    ...
    def recieved_marker_msg(self, marker_msg):
        if(len(marker_msg.markers) > 0):
            rospy.loginfo('You detected AR code.')
            
           # self.marker_pose = GripperMarkers._offset_pose(marker_msg.markers[0].pose.pose, 0, 0, 1.0)
            temp_marker_pose = marker_msg.markers[0].pose.pose
            #rospy.logwarn(str(temp_marker_pose))
            self.marker_pose_stamp = self.transform_to_stamp(temp_marker_pose, '/torso_lift_link', '/base_link')
            self.marker_pose = self.marker_pose_stamp.pose
            #rospy.logwarn(str(self.marker_pose))
#rospy.logwarn('init : ' + str(self.get_ee_pose()))
            #rospy.logwarn('ar code : ' + str(received_marker_msg.markers[0].pose.pose))
            self.move_marker_position()
            #self.move_head_to_marker()

    def move_head_to_marker(self):
        #head_pose = self.transform(self.marker_pose, )
        head_pose = self.marker_pose
        x = head_pose.position.x
        y = head_pose.position.y
        z = head_pose.position.z
        self.head_action(x,y,z)

    def move_marker_position(self):
        self.update_viz()
        ...
        if (ik_solution == None):
            rospy.loginfo('No IK solutions for this pose, cannot move.')
        else:
    ...
    def transform_to_stamp(self, pose, from_frame, to_frame):
        pose_stamped = PoseStamped()
        try:
            common_time = self.tf_listener.getLatestCommonTime(from_frame, to_frame)
            pose_stamped.header.stamp = common_time
    ...
    @staticmethod
    def _offset_pose(pose, x_offset = 0, y_offset = 0, z_offset = 0):
        transform = GripperMarkers.get_matrix_from_pose(pose)
        offset_array = [x_offset, y_offset, z_offset]
